...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (lat.equals("0") || lon.equals("0")) {
            gps.locationList.observe(viewLifecycleOwner) {
                if (!it.isNullOrEmpty()) {
                    viewModel.insertData(it[0], it[1], "minutely", unit, lang)
                }
            }
        ...
        viewModel.weatherApi.observe(viewLifecycleOwner) {
            if (lat.equals("0") || lon.equals("0")) {
                ...
                binding.cityName.text =
                    getCityName(gps.getLatitude()!!.toDouble(), gps.getLongitude()!!.toDouble())
            } else {
            ...
            if (it != null) {
                if (lang.equals("en")) {
                    ...
                    binding.txtVisibility.text =
                        convertNumbersToArabic(it.current.visibility.toInt())
                    binding.tempTxt.text = convertNumbersToArabic(it.current.temp.toInt()) + "°"
